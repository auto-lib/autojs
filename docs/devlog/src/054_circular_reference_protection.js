...
// 054: Circular Reference Protection in deep_equal
//
// CRITICAL FIX: The deep_equal function now handles circular references to prevent
// "Maximum call stack size exceeded" RangeError.
//
// The Issue:
//   When comparing objects with circular references (e.g., obj.child = obj, or
//   objects that reference each other: a.b = c; c.d = a), the previous deep_equal
//   implementation would recursively call itself infinitely, causing a stack overflow.
//
// The Solution:
//   Added a cache (Map) to track which object pairs we've already started comparing.
//   If we encounter the same pair again during recursion, we know it's a circular
//   reference and can safely return true (they're the same objects being compared).
//
// Examples of circular references that now work correctly:
//   - obj.self = obj
//   - a.b = c; c.d = a
//   - Arrays containing themselves: arr[0] = arr
//
// This maintains all previous features from 053 (root cause analysis, consolidated
// reporting, etc.) while fixing the critical stack overflow bug.
//
// Features:
//   1. Collection period - wait 2 seconds after first detection, gather all excessive functions
//   2. Static variable tracking - track all changes to static variables with diffs
//   3. Root cause analysis - trace backwards from affected functions to find which static
//      variables (things YOU set) are being changed too frequently
//   4. Dependency chains - show how changes propagate through the system
//   5. Value diffs - show what's actually changing on the root cause variables
//
// The report shows:
//   - Which static variable is the root cause (being set too frequently)
//   - Recent changes to that variable with old → new diffs
//   - All affected functions and their dependency chains from the root
//
// New options:
//   excessive_calls_collection_period: number (default: 2000ms) - how long to collect before reporting
//   static_value_history_size: number (default: 30) - how many changes to track per static var
...
let auto = (obj,opt) => {
    ...
    // Root cause analysis (NEW in 053)
    ...
    let static_value_history = {}; // track changes to static variables: { var_name: [{old, new, timestamp}, ...] }
    ...
    // == track changes to static variables (NEW in 053) ==

    let record_static_change = (name, old_val, new_val) => {
        // Only track static variables (things not in fn)
    ...
    // == find root static variables that caused a function to update ==
    ...
    let find_root_causes = (fn_name) => {
        ...
        let traverse = (name) => {
            ...
            // If it's a static variable (not a function), it's a root
            if (!(name in fn)) {
                roots.add(name);
                return;
            }

            // Otherwise traverse its dependencies
            if (deps[name]) {
    ...
    // == build dependency chain from root to target ==
    ...
    // == generate consolidated excessive calls report ==

    let generate_root_cause_report = () => {
        if (excessive_functions_collected.size === 0) return;

        // Find root causes for all excessive functions
        let root_cause_map = {}; // { root_var: [affected_functions] }
    ...
    // == check if function is being called too frequently (NEW in 053) ==
    ...
    let check_call_rate = (name) => {
        ...
        if (call_timestamps[name].length > max_calls_per_second) {
            if (!backed_off_functions[name]) {
                // Add to collection instead of immediately warning
    ...
    let update = (name,src,caller) => {
        ...
        // Check for excessive call rate after pushing to stack so we have proper context
    ...
    // Now with circular reference protection to prevent stack overflow
    let deep_equal = (a, b, cache) => {
        ...
        // Create cache on first call (top-level comparison)
        if (!cache) cache = new Map();

        // Check if we've already started comparing this object
        if (cache.has(a)) {
            // If we're comparing the same pair again, it's a circular reference
            // Return true if it's the same b we're comparing to
    ...
    let phase6_detect_changes = (triggers, sorted, old_values) => {
        ...
        sorted.forEach(name => {
            ...
            if (use_deep_equal && isObject) {
                // Use deep equality check for objects/arrays when enabled
                hasChanged = !deep_equal(old_val, new_val);
            } else if (isObject) {
                // Objects/arrays always count as changed (might be mutated) when deep_equal disabled
                hasChanged = true;
            } else {
