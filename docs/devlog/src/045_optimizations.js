...
// 045: Optimizations
//
// Building on 044's batched updates with performance and clarity improvements:
//
// 1. Reverse dependency map (dependents) - O(affected) instead of O(n) invalidation
// 2. Fix double subscription - subscriptions run once per transaction
// 3. Simplified circular detection - rely on topological sort
// 4. Fix async updates - no double propagation
// 5. Transaction-scoped trace - capture entire update transaction
// 6. Clearer separation - computation vs notification phases
...
let auto = (obj,opt) => {
    ...
    let propagate = (trigger_name, trigger_value) => {
        ...
        // Phase 5: Notify - run subscriptions for trigger + all changed values
        // Build list of all changed values (trigger + affected)
    ...
    let setter = (name, val) => {
        ...
        // (trace is created in propagate and includes full transaction)
        propagate(name, val);
    }
