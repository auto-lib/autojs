...
// 046: Batching and Transaction Metadata
//
// Foundation for recording/replay:
//
// 1. Batch API - Group multiple external sets into one transaction
//    $.batch(() => { $.data = x; $.filter = y; }) creates ONE transaction
//
// 2. Transaction metadata - IDs, timestamps, multiple triggers
//    trace = { id, timestamp, triggers: [{name, value}...], updates: {...} }
//
// 3. Simplified trace collection - Build updates at end instead of during propagation
...
let auto = (obj,opt) => {
    ...
    let setter = (name, val) => {
        ...
        // If we're in a batch, accumulate this change
        if (in_batch) {
            ...
            // Normal single-set operation - propagate immediately
            propagate({ name, value: val });
        }
    }
