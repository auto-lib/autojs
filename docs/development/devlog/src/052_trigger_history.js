...
// 052: Trigger History for Debugging Excessive Calls
//
// When excessive calls are detected, we now show:
//   1. The CURRENT trigger that caused this update (what just changed)
//   2. A rolling history of recent triggers for this function (pattern detection)
//
// This helps answer "WHY is this function being called so much?" immediately
// when the warning appears, instead of waiting for the backoff period to end.
//
// The trigger history shows:
//   - Which static variables triggered updates to this function
//   - Whether it's the same trigger repeatedly (likely a loop) or different triggers
//   - Old vs new values for each trigger
//
// New options:
//   trigger_history_size: number (default: 20) - how many triggers to keep per function
...
let auto = (obj,opt) => {
    ...
    // Trigger history for debugging (NEW in 052)
    let trigger_history = {}; // rolling history of triggers per function: { fn_name: [{name, value, old_value, timestamp}, ...] }
    ...
    // == record trigger in history for a function ==
    // == called when a function is about to be updated ==
    ...
    let record_trigger = (fn_name) => {
        ...
        // Find which of current_triggers are direct dependencies of this function
        ...
        // If no direct triggers, it's an indirect dependency - record the root trigger(s)
        if (dominated_triggers.length === 0 && current_triggers.length > 0) {
            // Record each root trigger separately so we can count them
            current_triggers.forEach(t => {
            ...
            // Add direct triggers to history
    ...
    // == format trigger history for display ==
    ...
    let check_call_rate = (name) => {
        ...
        if (call_timestamps[name].length > max_calls_per_second) {
            ...
            if (!backed_off_functions[name]) {
                ...
                // Show CURRENT trigger (NEW in 052)
                ...
                // Show trigger history (NEW in 052)
    ...
    let update = (name,src,caller) => {
        ...
        // Record trigger history BEFORE checking call rate (so we have context when warning fires)
        record_trigger(name);
