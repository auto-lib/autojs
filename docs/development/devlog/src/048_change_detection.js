...
// 048: Change Detection (Hybrid)
//
// Adds change detection to prevent unnecessary subscription fires:
//
// 1. Static values - In setter, check if PRIMITIVE values changed (===)
//    If no change, return early without propagating
//    Objects/arrays always propagate (might be mutated)
//
// 2. Computed values - After updating, compare old vs new
//    Primitives: Only fire subscriptions if value changed (===)
//    Objects/arrays: Always fire subscriptions (might be mutated)
//
// 3. Hybrid approach - Safe for both patterns:
//    - Immutable: $.items = [...$.items, item]  (new reference = detected)
//    - Mutable: $.items.push(item); $.items = $.items  (object = always propagates)
//
// Benefits:
// - Primitives optimized (numbers, strings, booleans, null)
// - Objects/arrays safe (mutations won't be missed)
// - Prevents UI re-renders when primitive values don't change
// - Works with both immutable and mutable coding styles
//
// Perfect for: preventing twitching in UIs, toggles, counters that don't change,
// filters that return same result, any case with primitive computed values
...
let auto = (obj,opt) => {
    ...
    // == Phase 4: Detect changes - only propagate what actually changed ==
    // == Phase 5: Notify - run subscriptions ==
    ...
    let propagate = (triggers) => {
        ...
        // Phase 4: Update - delete old values and mark for recomputation
        sorted.forEach(name => {
            if (name in value) {
                delete value[name];
            }
        ...
        // Phase 5: Recompute - trigger updates in dependency order
        sorted.forEach(name => {
        ...
        // Phase 6: Change detection - only include values that actually changed
        ...
        // Phase 7: Build updates map from final values (only changed ones)
        actually_changed.forEach(name => {
            trace.updates[name] = value[name];
        });

        // Phase 8: Notify - run subscriptions ONLY for values that actually changed
