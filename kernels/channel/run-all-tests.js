// Run all main tests against channel kernel
...
async function runTest(testPath, testName) {
    try {
        ...
        if (errors.length === 0) {
            ...
            return { name: testName, status: 'fail', errors, expected, actual };
        }
...
for (const file of files) {
    ...
    // Catch any unhandled errors during test
    let result;
    try {
        result = await runTest(testPath, testName);
        // Small delay to let async callbacks finish
        await new Promise(resolve => setTimeout(resolve, 10));
    } catch (err) {
        result = { name: testName, status: 'error', error: err.message };
    }
...
process.exit(results.fail + results.error > 0 ? 1 : 0);
